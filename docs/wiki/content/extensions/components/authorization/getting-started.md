...
`examples/vert` runs this exact scenario for real, with real ports and real tokens: `examples/vert/scripts/seed-authz-test-data.ts` and `examples/vert/scripts/seed-user-policies.ts` seed seven personas, and `examples/vert/scripts/test-authorization.sh` curls all of them - cases C1, C2, and C3 are the three requests above.
