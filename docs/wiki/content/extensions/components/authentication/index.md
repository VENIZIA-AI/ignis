...
- **One component, three auth mechanisms.** `AuthenticateComponent.binding()` reads `JWT_OPTIONS` (JWS or JWKS), `BASIC_OPTIONS` and `SERVICE_OPTIONS` from the DI container, and registers the token services their presence implies.
  - Bind at least one of the three - the component throws at startup if none is set. A service that only verifies assertions binds `SERVICE_OPTIONS` alone.
