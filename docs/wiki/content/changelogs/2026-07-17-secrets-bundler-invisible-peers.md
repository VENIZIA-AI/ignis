...
- **A missing peer now fails with the standard install hint.** Constructing `HashiCorpVaultHelper` or `DotenvVaultHelper` directly (via the `/vault` and `/dotenv-vault` sub-paths) without the peer installed used to reject with a raw module-not-found error; it now throws the same actionable `ApplicationError` (`Please install 'node-vault'`) as the factory path.
