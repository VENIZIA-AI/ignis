...
- **zod 4.5 gotcha, found downstream.** `z.object({ ...schema }).shape.optional()` loaded on 4.3.x by accident (the spread copied methods onto the plain object) and throws at module load on 4.5.x while `tsc` stays green. Derive from the schema itself: `Schema.omit({ ... }).partial().optional()`. See the [migration guide](/guides/migrations/boot-api-removal-migration).
