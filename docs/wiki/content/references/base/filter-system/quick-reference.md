...
| `order` (expressions) | `toOrderBy({ ..., expressions })` | `expressions: { total: computedSql }` | Sorts a joined column or a computed `SQL`, matched by its own key in `expressions` first |
...
- [`packages/connectors/src/relational/postgres/repositories/dialect/query.ts`](https://github.com/VENIZIA-AI/ignis/blob/main/packages/connectors/src/relational/postgres/repositories/dialect/query.ts) - `PostgresQueryOperators.FNS`, one handler per operator
