...
- **Health Check Endpoint:** Exposes a GET endpoint (by default at `/health`) that returns a JSON response indicating the application's status.
- **Configurable Path:** The path of the health check endpoint can be configured.
...
```typescript
import { HealthCheckBindingKeys, IHealthCheckOptions } from '@vez/ignis';
