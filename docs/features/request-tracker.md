...
- **Request Logging:** Logs the start and end of each request, including the request method, path, URL, and the time taken to process the request.
- **Request ID:** Adds a unique request ID to each incoming request. This ID is available in the context and can be used in subsequent logs to correlate all log entries for a specific request.
- **IP Forwarding:** It can log the real IP address of the client by looking at the `x-real-ip` or `x-forwarded-for` headers.
...
```typescript
// src/application.ts
import { RequestTrackerComponent } from '@vez/ignis';
