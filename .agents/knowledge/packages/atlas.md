...
- **Dates decide the window, not commits.** `changes` translates two versions into the dates they
  shipped on, then takes `fromDate < date <= toDate` - the lower bound open, the upper closed. Two
  releases on one day therefore leave an empty window, and the default window (newest release, the
  one before it) can be empty for that reason.
- **A changelog with no package resolves to none.** Package inference tries a `packages:`
  frontmatter list, then a Details table's `Package` column, then `@venizia/ignis...` mentions in
  the body; the first rule that hits wins. A file none of them answer for keeps an empty
  `packages`, so a package filter never returns it - 27 of 118 files today. The generator never
  guesses.
