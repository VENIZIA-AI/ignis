...
Every scope owns its folder with an `index.ts` barrel, and nests its own `common/{types,constants}.ts` behind a `common/index.ts` barrel - contracts live in `types.ts`, free of concrete classes, which is what keeps `binding/` and `container/` decoupled. This is the template every other IGNIS package's folder layout follows.
