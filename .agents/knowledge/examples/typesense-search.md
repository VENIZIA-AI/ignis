...
- **A hand-written full-text search endpoint** - `GET /search` calls `ArticleRepository.search()`, a raw passthrough to the Typesense driver with no `TFilter` translation and, notably, no `@model` `defaultFilter` applied - so it can see `draft`/`archived` articles that the CRUD routes cannot. The example calls this gap out explicitly rather than hiding it.
