...
**Application / BaseApplication** - `BaseApplication` in `packages/core/src/base/applications/base.ts`
...
**Binding** - `packages/inversion/src/binding/binding.ts`. The fluent object wrapping one
...
**Component** - `BaseComponent` in `packages/core/src/base/components/base.ts`. A pluggable unit an
...
**Connector** - A paradigm-specific implementation family under `packages/core/src/connectors/*`
(`postgres`, `search`, `typesense`, `meilisearch`) providing the DataSource and repository chain for
one storage engine.

**Container** - `Container` in `packages/inversion/src/container/container.ts`, built on
`BaseContainer` -> `AbstractContainer`. Holds all `Binding`s, resolves instances, and drives
constructor/property injection. `AbstractApplication` extends it directly.
...
**DataSource** - `AbstractDataSource` in `packages/core/src/base/datasources/abstract.ts`. An
...
**Filter** - The query-shape Zod schemas in `packages/core/src/base/repositories/query-schemas/`
(`where`, `fields`, `limit`, `offset`, `order-by`, `skip`, `include`) that a repository's find
methods accept.

**Helper** - Any production-ready utility class under `packages/helpers/src/modules/` (Logger,
Redis, Queue, Storage, Crypto, Cron, Socket.IO, Network, UID, Worker) - always extending `BaseHelper`.

**Injection (`@inject`)** - The parameter/property decorator in
`packages/inversion/src/metadata/injectors.ts` that records a binding key for the container to
resolve at instantiation time. Every constructor parameter of a container-instantiated class must
...
`packages/core/src/base/mixins/types.ts`) composed onto `BaseApplication` to add `component()`,
...
**Model** - An `AbstractEntity` subclass (`packages/core/src/base/models/base.ts`) pairing a name
...
**Provider** - `BaseProvider<T>` in `packages/core/src/base/providers/base.ts`, implementing
`IProvider<T>` - a class that produces a bound value (e.g. a middleware handler) rather than being
injected directly.

**Repository** - `AbstractRepository` in `packages/core/src/base/repositories/core/abstract.ts`, the
...
`BindingScopes.TRANSIENT` (`packages/inversion/src/binding/common/constants.ts`). Singleton caches
the resolved instance; transient resolves fresh every time.
...
(`packages/core/src/base/datasources/abstract.ts`), passed through repository calls as
