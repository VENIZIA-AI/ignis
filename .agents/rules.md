...
| ID | Rule |
|---|---|
| **P-01** | **Read the knowledge bundle first.** Start at [`knowledge/index.md`](knowledge/index.md), open the concepts your task touches, then act. Query it with the `ignis-atlas` MCP (`search` with `corpus: "knowledge"`, `get`). The code is ground truth: a concept that disagrees with the code is a bug, fix it. |
