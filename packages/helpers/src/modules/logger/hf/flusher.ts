...
export class HfLogFlusher {
  ...
  constructor(options?: IHfLogFlusherOptions) {
    this.sink = options?.sink ?? HfLogFlusher.buildDefaultSink(options?.filePath);
  ...
  private static renderEntry(ring: TRingState, sequence: number): string {
  ...
  private async drain(): Promise<void> {
    ...
    while (this.flushIndex < ring.writeIndex) {
      ...
      for (let index = 0; index < count; index++) {
        lines.push(HfLogFlusher.renderEntry(ring, this.flushIndex++));
      }
