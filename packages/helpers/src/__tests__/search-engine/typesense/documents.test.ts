...
describe('TypesenseHelper documents', () => {
  ...
  test('importDocuments rejects an invalid action', async () => {
    ...
    try {
      await helper.importDocuments({
        collection: 'products',
        documents: [{ id: '1' }],
        // Untyped-caller path: the compile-time guard now rejects this, so force it through.
  ...
  test('deleteAllDocuments uses the native truncate primitive', async () => {
    ...
    // truncate delete: record('documents.delete', name, id=undefined, params) => args[2] is the params
