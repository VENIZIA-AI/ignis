// Minimal hand-rolled stand-in for the Typesense Client surface the helper uses.
...
export const createFakeClient = (behavior: IFakeBehavior = {}) => {
  ...
  const documents = (name: string, id?: string) => ({
    ...
    import: async (docs: unknown[], params?: unknown) => {
      ...
      return behavior.importResponse ?? (docs as unknown[]).map(() => ({ success: true }));
    },
...
// Shared helper-construction fixture — the single source of truth for test suites (do NOT
