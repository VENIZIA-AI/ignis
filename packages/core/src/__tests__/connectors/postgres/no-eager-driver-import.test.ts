...
describe('optional and sub-path-only modules are never eagerly loaded', () => {
  const ENTRIES = [
    ...
    { name: '@venizia/ignis/sqlite', entry: 'src/connectors/sqlite/index.ts' },
  ];
  ...
  const FORBIDDEN = [
    ...
    // The SQLite driver peer: `@venizia/ignis/sqlite` must stay
  ...
  test('the pglite sub-path really does load @electric-sql/pglite', async () => {
    ...
    const loaded = await countDriverModules({
      entry: 'src/connectors/postgres/drivers/pglite.ts',
  ...
  test('the libsql sub-path really does load @libsql/client', async () => {
    ...
    const loaded = await countDriverModules({
      entry: 'src/connectors/sqlite/drivers/libsql.ts',
