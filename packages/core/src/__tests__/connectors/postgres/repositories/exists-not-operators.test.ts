...
/**
 * `exists`/`notExists`/`not` are in the neutral QueryOperators vocabulary but previously had no
 * Postgres handler, so `{ deletedAt: { exists: false } }` threw 'Invalid query operator'.
 */
