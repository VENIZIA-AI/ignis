...
/**
 * Compile-time contract: bulk writes on search engines are count-only. `shouldReturn` is
 * rejected at the TYPE level for search updateAll/deleteAll, while postgres keeps the full
 * shouldReturn overloads untouched.
 */
