...
/** Shared engine-stubbing for every probe (same pattern as shared-core.test.ts): pass-through
 * default filter/limit and a canned count, so retry behavior is observable without a database.
 * Each subclass supplies only what its scenario varies. */
...
/** Keeps the REAL `validateLockOptions` - the lock/transaction invariant is invisible to
 * `RetryProbeRepository` because that class no-ops the validator. */
...
/** Compile-time contract - never executed. Write verbs must reject `retry`: deleting any
 * of the ts-expect-error markers below must break the build. */
