...
import { ISearchConnector } from '@/search/core';
import { ISearchDataSourceOptions } from '@/search/core/datasources';
...
import { ISearchQueryDialect, SearchFilterOutcomes } from '@/search/core/repositories/common';
...
describe('BaseSearchDataSource - discoverCollections / getSchema', () => {
  ...
  test('@repository bindings were registered for every fixture repository', () => {
    ...
    expect(registry.getRepositoryBinding({ name: SearchProductRepository.name })).toBeDefined();
...
describe('BaseSearchDataSource - provisionCollections', () => {
  ...
  test('@repository bindings were registered for both fixture repositories', () => {
    ...
    expect(registry.getRepositoryBinding({ name: SearchWidgetRepository.name })).toBeDefined();
...
describe('BaseSearchDataSource - paradigm seam (search/core imports no engine)', () => {
  test('no file under src/search/core imports typesense or meilisearch', async () => {
    for (const file of listTsFiles('src/search/core')) {
      ...
      expect(content).not.toContain('search/meilisearch');
