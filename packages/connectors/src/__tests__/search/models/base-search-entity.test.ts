...
describe('BaseSearchEntity', () => {
  ...
  test('getSchema SELECT derives from the DSL and parses a full document', () => {
    const doc = new ProductDocument();
    const schema = doc.getSchema<z.ZodType>({ type: SchemaTypes.SELECT });
  ...
  test('getSchema UPDATE accepts a partial document', () => {
    const doc = new ProductDocument();
    const schema = doc.getSchema<z.ZodType>({ type: SchemaTypes.UPDATE });
  ...
  test('getSchema caches derived schemas per (class, type)', () => {
    const doc = new ProductDocument();
    const first = doc.getSchema<z.ZodType>({ type: SchemaTypes.SELECT });
    const second = doc.getSchema<z.ZodType>({ type: SchemaTypes.SELECT });
  ...
  test('constructor accepts an explicit schema override', () => {
    ...
    expect(
      doc.getSchema<z.ZodType>({ type: SchemaTypes.SELECT }).safeParse({ id: '1', sku: 'abc' })
        .success,
    ).toBe(true);
  });
  ...
  test('documentSchema override wins wholesale over DSL derivation', () => {
    ...
    const selectSchema = doc.getSchema<z.ZodType>({ type: SchemaTypes.SELECT });
    const updateSchema = doc.getSchema<z.ZodType>({ type: SchemaTypes.UPDATE });
  ...
  test('missing definition throws on first access of getSchema', () => {
    ...
    expect(() => doc.getSchema<z.ZodType>({ type: SchemaTypes.SELECT })).toThrow(
