...
/**
 * Registers these codes with the shared key registry so a consumer gets autocomplete on
 * `messageCode`. The empty type-only import is what puts the root barrel in the program so the
 * augmentation below resolves - it is erased at build, so it costs the browser bundle nothing. Any
 * value import of that barrel would pull `ioredis` and fail a browser build on `tls`.
 */
import type {} from '@venizia/ignis-helpers';
