...
> implements TSqliteDriver<Schema, Client> {
  ...
  constructor(opts: TLibSqlDriverOptions) {
    ...
    this.client = client;
    // Not `LoggerFactory`: it reaches the helpers root barrel, which pulls `ioredis` and fails a
    // browser bundle on `tls`.
    this.logger = new BaseHelper({ scope: LibSqlDriver.name }).getLogger();
  ...
  async acquire(opts: { schema: Schema }): Promise<TSqliteConnection<Schema>> {
    ...
    return {
      ...
      query: async <R>(queryOpts: { statement: string }) => {
        const result = await client.execute(queryOpts.statement);
