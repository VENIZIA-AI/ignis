...
const bootTrackedApplication = async (): Promise<OpenAPIHono> => {
  ...
  // The real stack, not a hand-rolled one: `requestId()` and the error handler are installed by
  // `registerDefaultMiddlewares()`, and reconstructing them here is how a test drifts from the
  // framework it is meant to be checking.
